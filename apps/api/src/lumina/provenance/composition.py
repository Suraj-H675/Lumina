...
def production_provider_registry(*, repository_root: Path | None = None) -> StaticProviderRegistry:
    """Construct the sole production registration without performing network I/O."""
    config = nasa_runtime_config(repository_root=repository_root)
    adapter = NasaExoplanetArchiveAdapter(
        BoundedHttpTransport(timeout=config.timeout),
...
def compose_provider_runtime(
    ...
    *,
    ...
    registry = production_provider_registry(repository_root=repository_root)
    sync_service = ProviderSyncService(
        registry=registry,
