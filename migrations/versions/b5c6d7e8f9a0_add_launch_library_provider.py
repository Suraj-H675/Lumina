"""Provision the disabled Phase 4C Launch Library 2 provider."""
...
_PROVIDER_CODE: Final = "launch-library-2"
_SAFE_ERROR: Final = "Launch Library 2 provider migration precondition failed."
...
def upgrade() -> None:
    """Add exactly one disabled Launch Library 2 runtime row."""
...
def downgrade() -> None:
    """Remove Launch Library 2 only while its disabled initial state remains untouched."""
    connection = _connection()
    identity = _identity()
    _assert_actor(connection, identity)
