"""Provision the disabled Phase 4D CelesTrak GP provider."""
...
_PROVIDER_CODE: Final = "celestrak-gp"
_SAFE_ERROR: Final = "CelesTrak provider migration precondition failed."
...
def upgrade() -> None:
    """Add exactly one disabled CelesTrak runtime row."""
...
def downgrade() -> None:
    """Remove CelesTrak only while its disabled initial state remains untouched."""
    connection = _connection()
    identity = _identity()
    _assert_actor(connection, identity)
