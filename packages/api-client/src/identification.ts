...
import { zIdentificationCreateResponse, zIdentificationStatusResponse } from "./generated/zod.gen";
...
async function readBoundedJson(response: Response, controller: AbortController): Promise<unknown> {
  if (!isJsonMediaType(response.headers.get("content-type")) || !declaredBodyIsBounded(response)) {
...
export async function createIdentificationSubmission(
  ...
  const outcome = await runTimed(timeoutMs, options.signal, async (signal, controller) => {
    ...
    try {
      const raw = await readBoundedJson(response, controller);
